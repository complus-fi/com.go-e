...
- [x] **Cloud `onInit` copy-paste divergence** — `drivers/evcharger-cloud-device.js:16-46`
  ...
  **Fix:** refactor cloud `onInit` to call `await super.onInit()`, then apply only the
  cloud-specific bits (base_url from `serialnumber`, `cloud_api_token`, and the
  `clearIntervals()` + `onPoll()` kick-off that replaces mDNS `onDiscoveryAvailable`). Removes
  the duplication *and* restores the missing initializers.

## P2 — Dead code removal

- [ ] **`lib/go-eCharger-API-v2.js`** — remove unused power-conversion / single-write block:
...
- [ ] **`lib/mappings.js`** — remove `rfidCards` (:711, exported :831, never consumed),
...
- [ ] **`goe_pv_surplus_enabled` — fully dead capability path.** Declared in **no** compose
...
- [ ] **`'fst'` in `orderedKeys`** — `drivers/evcharger-device.js:617`. No mapping emits an `fst`
  key; remove the unused ordering slot.

## P3 — Redundancy / optimization

- [ ] **Recompute-per-poll:** `getConfiguredTransactionEntries` (`evcharger-device.js:260`) is
...
- [ ] **Map-then-override waste:** `mapStatusToCapabilities` computes `nextValues.goe_transaction`
...
- [ ] **Minor timer nits (`evcharger-device.js`):** `clearIntervals` (:467) clears the interval
