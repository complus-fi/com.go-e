...
5. API command ordering prioritizes `frc`, then `amp`, then `psm`.

## Capability Matrix

| Homey capability         | Direction                      | API keys used           | Mapping behavior                                                          | Notes                                                                        |
| ------------------------ | ------------------------------ | ----------------------- | ------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| evcharger_charging       | Read + write                   | alw (read), frc (write) | Read: boolean charging allowed. Write false -> frc=1, write true -> frc=2 | Write mapping defined from `alw` entry in [lib/mappings.js](lib/mappings.js) |
...
- If charging is turned off, command flow sends force-off behavior.
...
- Control semantics change for `frc`, `amp`, or `psm`.
