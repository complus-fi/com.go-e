...
class goeChargerAPI {
  /**
   * @param {string} api_base_url Charger API base URL (for example `http://192.168.1.50/api`).
   ...
   */
  constructor(api_base_url, driver, apiKeys) {
    this.api_base_url = typeof api_base_url === 'string' ? api_base_url.replace(/\/+$/, '') : null;
    this.driver = driver;
  ...
  normalizeApiError(action, error) {
    ...
    return new Error(`go-eCharger ${action} failed at ${this.api_base_url}: ${message}${suffix}`);
  }
  ...
  async testConnection() {
    try {
      const response = await fetch(`${this.api_base_url}/status?filter=fna`, { method: 'GET' });
      return response.ok;
    } catch {
      return false;
    }
  ...
  async getStatus() {
    ...
    const url = `${this.api_base_url}/status?filter=${filter}`;
  ...
  async setValue(key, value) {
    try {
      ...
      const url = `${this.api_base_url}/set?${key}=${encodeURIComponent(serializedValue)}`;
  ...
  async setValues(values = {}) {
    try {
      ...
      const url = `${this.api_base_url}/set?${query}`;
