...
// ------------------------------------------------------------------------
  ...
  async _getInfoAPIV2(includeCableLimit = false) {
    const deviceInfo = {
      id: '',
      name: '',
      status: 'unknown',
    ...
    // cbl is requested only when the driver/device exposes cable_limit capability.
    const statusFilter = ['sse', 'fna', 'car', 'alw', 'acs', 'frc', 'trx', 'cards', 'err', 'wh', 'pha', 'nrg', 'eto', 'tma', 'amp', 'ama', 'fsp'];
    if (includeCableLimit) statusFilter.push('cbl');

    const res = await this._getGoeCharger(`api/status?filter=${statusFilter.join(',')}`);
    ...
    switch (goecharger.trx) {
      case 0:
        deviceInfo.transaction = 'card_0';
        break;
      case 1:
        deviceInfo.transaction = 'card_1';
        break;
      case 2:
        deviceInfo.transaction = 'card_2';
        break;
      case 3:
        deviceInfo.transaction = 'card_3';
        break;
      case 4:
        deviceInfo.transaction = 'card_4';
        break;
      case 5:
        deviceInfo.transaction = 'card_5';
        break;
      case 6:
        deviceInfo.transaction = 'card_6';
        break;
      case 7:
        deviceInfo.transaction = 'card_7';
        break;
      case 8:
        deviceInfo.transaction = 'card_8';
        break;
      case 9:
        deviceInfo.transaction = 'card_9';
        break;
      case 10:
        deviceInfo.transaction = 'card_10';
        break;
      default:
        deviceInfo.transaction = 'card_none';
    }
    ...
    if (includeCableLimit && goecharger.cbl !== null) deviceInfo.cable_limit = Number(goecharger.cbl);
    ...
    // cards information (V2 API) - handle both old cards array and new individual attributes from firmware 60.0 onwards
    if (goecharger.cards && Array.isArray(goecharger.cards)) {
      // Old API format with cards array
      deviceInfo.cards = goecharger.cards.map((card, index) => ({
        id: index + 1,
        name: card.name,
        energy: card.energy,
        energyKwh: Number((card.energy / 1000).toFixed(3)),
        isConfigured: card.cardId
      }));
    } else {
      // New API format replaced cards array in firmware 60.0 with individual card attributes cXn, cXe, cXi
      deviceInfo.cards = [];
      for (let i = 0; i <= 9; i++) {
        const nameKey = `c${i}n`;
        const energyKey = `c${i}e`;
        const idKey = `c${i}i`;

        // Check if any of the card attributes exist for this card index
        if (goecharger.hasOwnProperty(nameKey) || goecharger.hasOwnProperty(energyKey) || goecharger.hasOwnProperty(idKey)) {
          deviceInfo.cards.push({
            id: i + 1,
            name: goecharger[nameKey] || 'n/a',
            energy: goecharger[energyKey] || 0,
            energyKwh: Number(((goecharger[energyKey] || 0) / 1000).toFixed(3)),
            isConfigured: Boolean(goecharger[idKey])
          });
        } else {
          // Add default card entry if no attributes found
          deviceInfo.cards.push({
            id: i + 1,
            name: 'n/a',
            energy: 0,
