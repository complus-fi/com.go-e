 ...
 */
const { isVersionAtLeast, round } = require('./helpers');

const PHASE_CONTROL_PSM_MIN_VERSION = '60.4';

function selectPhaseAPIkeys(context = {}) {
  return isVersionAtLeast(context.firmwareVersion, PHASE_CONTROL_PSM_MIN_VERSION);
}
...
const capabilityMap = {
  ...
  target_power: {
    apiKeys: ['amp', 'fsp'],
    apiKeysForContext: (context = {}) => (selectPhaseAPIkeys(context) ? ['amp', 'psm'] : ['amp', 'fsp']),
    requiredApiKeys: ['amp'],
    homeyCapabilities: ['target_power'],
    apiToHomey: (status, api) => {
      let isSinglePhase;

      if (status.psm !== undefined && status.psm !== null) {
        // Firmware 60.4+: psm 1=single phase, 2=three phase, 0=automatic.
        isSinglePhase = Number(status.psm) === 1;
      } else if (status.fsp !== undefined && status.fsp !== null) {
        // Legacy firmware: fsp boolean-like flag.
        isSinglePhase = Boolean(status.fsp);
      } else {
        return {};
      }
    ...
    homeyToApi: (values, getCapValue, context = {}) => {
      ...
      // Firmware 60.4+: replace fsp with psm (1=single phase, 2=three phase).
      if (selectPhaseAPIkeys(context) && config.fsp !== undefined) {
  ...
  target_power_mode: {
    ...
    homeyToApi: (values, getCapValue, context = {}) => {
      ...
      return selectPhaseAPIkeys(context) ? { frc: 0, psm: 0 } : { frc: 0 };
    }
  },
  nrg: {
    apiKeys: ['nrg', 'pha'],
