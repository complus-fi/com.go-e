  ...
  target_power: {
    ...
    apiToHomey: (status) => {
      ...
      if (!Number.isFinite(amp) || amp <= 0) {
        return { target_power: 0 };
      }

      const phaseVoltages = status.nrg.slice(0, 3).map((value) => Number(value));

      let phaseCount = Number(status.pnp);
      if (![1, 3].includes(phaseCount)) {
        const activeByVoltage = phaseVoltages.filter((value) => Number.isFinite(value) && value > 0).length;
        phaseCount = activeByVoltage >= 2 ? 3 : 1;
      }

      if (phaseCount === 1) {
        const activeIndex = phaseVoltages.findIndex((value) => Number.isFinite(value) && value > 0);
        if (activeIndex === -1) return {};

        const activeVoltage = phaseVoltages[activeIndex];
        if (!Number.isFinite(activeVoltage) || activeVoltage <= 0) return {};

        return { target_power: Math.max(0, Math.round(amp * activeVoltage)) };
      }

      const validVoltages = phaseVoltages.filter((value) => Number.isFinite(value) && value > 0);
      if (validVoltages.length === 0) return {};

      const combinedPhaseVoltage = validVoltages.reduce((sum, value) => sum + value, 0) / validVoltages.length;
      const lineVoltage = combinedPhaseVoltage * Math.sqrt(3);
      const watts = Math.round(amp * lineVoltage * Math.sqrt(3));

      return { target_power: Math.max(0, watts) };
    }
  },
  fup: [
    'target_power_mode',
    // fup=true means charger automatic PV surplus logic is active.
    (value) => (Boolean(value) ? 'device' : 'homey'),
    (value, getCapValue, context = {}) => {
      if (value === 'device') {
        return {
          lmo: 4,
          fup: true,
