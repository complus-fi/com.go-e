...
function hasCardSpecificCapabilities(capabilities = []) {
  return capabilities.some((capability) => {
    return /^meter_power\.(10|[1-9])$/.test(capability) || /^goe_meter_power_name\.(10|[1-9])$/.test(capability);
  });
}
...
const capabilityMap = {
  ...
  atp: {
    apiKeys: ['atp'],
    requiredApiKeys: [],
    homeyCapabilities: ['alarm_problem.time'],
    apiToHomey: (status) => ({
      'alarm_problem.time': status.atp?.notEnoughActivated === true
    })
  },
  eto: ['meter_power', (value) => (Number(value) > 0 ? Number(value) / 1000 : null)],
  wh: ['meter_power.session', (value) => Number(value) / 1000],
  tma: {
    apiKeys: ['tma'],
    ...
    apiToHomey: (status) => {
      ...
      return { measure_temperature: average };
    }
  },
  goe_charger_mode: {
    apiKeys: ['lmo', 'fup', 'awe'],
  ...
  pakku: {
    ...
    apiToHomey: (status) => {
      ...
      return {
        'measure_power.pakku': Number.isFinite(parsed) ? parsed : 0
      };
    }
  },
  pgrid: {
    ...
    apiToHomey: (status) => {
      ...
      return {
        'measure_power.pgrid': Number.isFinite(parsed) ? parsed : 0
      };
    }
  },
  ppv: {
    ...
    apiToHomey: (status) => {
      ...
      return {
        'measure_power.ppv': Number.isFinite(parsed) ? parsed : 0
      };
    }
  },
  awcp: {
  ...
  nrg: {
    ...
    apiToHomey: (status) => {
      ...
      mapped.measure_power = Number.isFinite(totalPower) ? totalPower : 0;
      ...
      if (numPhases > 0) {
        ...
        mapped.measure_current = avgCurrent;
      } else {
      ...
      if (Array.isArray(status.pha)) {
        ...
        mapped.measure_voltage = nInPhases === 3 ? (vInSum / nInPhases) * Math.sqrt(3) : vInSum || Number(status.nrg[3]);
        ...
        if (nOutPhases === 3) {
          mapped['measure_voltage.output'] = (vOutSum / nOutPhases) * Math.sqrt(3);
        } else if (nOutPhases > 0) {
          mapped['measure_voltage.output'] = vOutSum;
        } else {
...
function mapStatusToCapabilities(status, capabilities = [], api) {
  ...
  for (let cardIndex = 1; cardIndex <= 10; cardIndex += 1) {
    ...
    if (capabilities.includes(energyCapability)) {
      ...
      mappedValues[energyCapability] = Number.isFinite(energyWh) ? energyWh / 1000 : 0;
    }
...
const deviceTypeIconMap = [
  ...
  { devicetype: 'go-eCharger', icon: '../../assets/go-echarger_homeplus.svg' }
];

const modelDiscoveryMatchers = {
  homeplus: ({ devicetype }) => devicetype === 'go-eCharger',
