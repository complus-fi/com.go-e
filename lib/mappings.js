...
const capabilityMap = {
  ...
  frc: {
    ...
    apiToHomey: (status) => {
      const frc = Number(status.frc);
      if (!Number.isFinite(frc)) return {};

      if (frc === 0) {
        return { target_power_mode: 'device' };
      }
      ...
      if (frc === 2) {
        return { target_power_mode: 'homey' };
      }
  ...
  target_power: {
    ...
    apiToHomey: (status) => {
      ...
      if (!Number.isFinite(totalPower)) return {};

      return { target_power: Math.max(0, totalPower) };
    }
  },
