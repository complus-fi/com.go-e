...
const capabilityMap = {
  ...
  frc: [
    'target_power_mode',
    // frc=0 means neutral/device-controlled; 1 or 2 means Homey is in control.
    (value) => (Number(value) === 0 ? 'device' : 'homey')
  ],
  nrg: {
    apiKeys: ['nrg', 'pha'],
...
const deviceTypeIconMap = [
  { devicetype: 'go-eCharger_Phoenix', devicesubtype: /^pro/i, icon: 'go-echarger_pro.svg' },
  { devicetype: 'go-eCharger_Pro', icon: 'go-echarger_core.svg' },
  { devicetype: 'go-eCharger_V4', icon: 'go-echarger_gemini.svg' },
  { devicetype: 'go-eCharger_V3', icon: 'go-echarger_homeplus.svg' }
];

const modelDiscoveryMatchers = {
  home: ({ devicetype }) => devicetype === 'go-eCharger_V3',
