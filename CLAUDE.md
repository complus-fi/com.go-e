...
- This app is the **sole writer** of `pgrid` (and the rest of `ids`), via
  `onCapability_SET_PV_SURPLUS_INFO` (the `set_pv_surplus_info` flow action). The feed is a Homey
  flow that reads the P1 house-grid meter and pushes `pgrid` on **every P1 power change** — it is
  change-triggered, not a fixed cadence, so it legitimately goes quiet during steady-state grid.
...
- PV is therefore attributed **only when `pgrid` is fresh**: a push must have arrived within
  `PV_SURPLUS_STALE_MS` (5 min). The push timestamp is `this.lastPvSurplusInfoTs`, stamped on each
  successful `onCapability_SET_PV_SURPLUS_INFO`.
- `PV_SURPLUS_STALE_MS` is deliberately **much longer than `PV_RATIO_WINDOW_MS`** (the 1-min
  smoothing window). A change-triggered feed (e.g. a flow pushing on every P1-meter power change)
  legitimately goes quiet for minutes during steady-state surplus charging, where grid power barely
  moves — the echoed `pgrid` stays valid across those gaps, so a 1-min gate would wrongly zero the
  ratio. The gate only needs to catch the feed dying entirely (overnight), which is minutes-to-hours.
  Do **not** re-tie the stale window to the averaging window.
...
- `pgrid ≈ 0` is the **normal steady state of working surplus charging** (surplus charging drives
  net grid toward zero). Never treat `pgrid == 0` itself as "stale/no PV" — only elapsed time since
  the last push tells staleness apart from a genuinely balanced grid.
